...
def train_model(X_train_tfidf: list, y_train: list, learning_rate: float,
                max_iter: int, batch_size: int, C: float,
                random_state: int, lr_decay: float = 1.0):
    print("\n[4/5] Training model...")
...
def evaluate_model(model, X_test_tfidf: list, y_test: list):
    ...
    if model.loss_history:
        ...
        for epoch, loss in enumerate(model.loss_history, 1):
            bar_len = int((loss / max_loss) * 60) if max_loss > 0 else 0
            print(f"    Epoch {epoch:>3}: {loss:.6f}  {'█' * bar_len}")
